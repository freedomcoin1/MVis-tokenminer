...
void gpu_init()
{
  ...
  if( inf )
  {
    ...
    printf ("\n\nRead ./FDT.conf - using custom intensity %d and CUDA device %d.", intensity, cuda_device);
  }
  else
  {
    intensity = INTENSITY;
    cuda_device = CUDA_DEVICE;
    printf ("\n\nNo ./FDT.conf - using hardcoded intensity %d and CUDA device %d.", intensity, cuda_device);
  }
